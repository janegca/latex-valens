% Valens Book V.01.1 53rd example chart GHL121
\documentclass[border=4pt]{standalone}
\input{chartPreamble}

\begin{document}
\begin{tikzpicture}[scale=3.5]
\begin{scope}[rotate=165]
	% draw numbered, fixed house positions
	\foreach \sign/\label in {
		0 / 1, 
		30/ 2, 
		60 / 3,
		90 / 4, 
		120 / 5, 
		150 / 6,
		180/ 7 , 
		210 / 8, 
		240 / 9,
		270 + 5 / 10, 
		300 + 5 / 11, 
		330 + 5/ 12}
	\draw (\sign + 15:0.2) node {\tiny\label};
\end{scope}

\begin{scope}[rotate=-15]
	% --- Draw the basic chart
	% draw the circles
	\draw[thick] (0,0) circle(1.05);
	\draw[thick] (0,0) circle(0.85);
	
	% draw cusps
	\foreach \x in {0,30,...,330} {
	        % lines from center to point
	        \draw[gray] (0,0) -- (\x:1.05);
	}
	
	% draw center of circle
	\fill[white] (0,0) circle(0.15);
	\draw (0,0) circle(0.15);
	\draw (0,0) circle(0.25);
	
	% draw sign labels
	\foreach \sign/\label in {
	15 /\Aries, 45 /\Taurus, 75 /\Gemini,
	105 /\Cancer, 135 /\Leo, 165 /\Virgo,
	195 /\Libra, 225 /\Scorpio, 255 /\Sagittarius,
	285 /\Capricorn, 315 /\Aquarius, 345 /\Pisces}
	\draw (\sign:0.93) node {\label};
	
	% --- draw Asc and MC lines
	\draw [->, red] 
	    (15:1.2)--(195:1.2cm)
	    node [above] {\ASC}; 
	
	% --- draw planets
	\node         at (215:0.75) {\Sun};      
	\node         at (195:0.75) {\Mercury};   
	
	\node         at (188:0.75) {\Moon};     
	\node         at (205:0.75){\Venus};    
		
	\node         at (123:0.75) {\Saturn};
	
	\node         at (230:0.75) {\Jupiter};  	
	\node         at (276:0.75) {\Mars};     
	
	% -- Placements given in Greek Horoscopes
	\node	[{red}] at (156:0.75)  {[\Mars]};
	
	\pic at (165:0.6){lot=A};

\end{scope}
\end{tikzpicture}
\end{document}
